...
class CustomDatePicker:UIView {
    ...
    func setupView(frame:CGRect){
        ...
        datePicker.addTarget(self, action: #selector(CustomDatePicker.chooseValue(_:)), forControlEvents:.ValueChanged)
        //分割线
        ...
        //底部按钮
        let cancelBtn = createBtnsWith(CGRectMake(0, datePickerHeight, btnWidth, 60), title: "取消", action: #selector(CustomDatePicker.clickCancel(_:)))
        let OKBtn = createBtnsWith(CGRectMake(btnWidth, datePickerHeight, btnWidth, 60), title: "确定", action: #selector(CustomDatePicker.clickOK(_:)))
