...
class MainView: UIView {
    ...
    private func setupTopBar(frame:CGRect){
        ...
        icon = UIButton(frame: CGRectMake(IconMarginLeft, IconMarginVertical, IconWidth, IconWidth))
        icon.setImage(UIImage(named: "head_icon"), forState: .Normal)
        
        title = UILabel(frame: CGRectMake(TitleX, 0, TitleWidth, frame.height))
        title.textAlignment = .Center
        title.text = "未登录"
        
        upload = UIButton(frame: CGRectMake(SettingX, SettingY, SettingWidth, SettingWidth))
        upload.setImage(UIImage(named: "upload"), forState: .Normal)
